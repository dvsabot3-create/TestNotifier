...
// Import AuthManager if available, otherwise create mock
let AuthManager;
try {
  // Try to load from auth-manager.js if it exists
  if (typeof importScripts !== 'undefined') {
    importScripts('auth-manager.js');
  }
  AuthManager = self.AuthManager || class AuthManager {
    constructor() {
    ...
    async initialize() {
      console.log('⚠️ Using mock AuthManager - auth disabled');
      return true;
    }

    async verifySubscription() {
      return { tier: 'premium', isActive: true };
    }
  };
} catch (error) {
  console.log('⚠️ AuthManager not available, using mock implementation');

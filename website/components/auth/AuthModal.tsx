...
export function AuthModal({ isOpen, onClose, redirectUrl, source }: AuthModalProps) {
  ...
  const handleGoogleAuth = async () => {
    ...
    try {
      ...
      // Get backend URL - check environment variable first, then fallback
      const getBackendUrl = () => {
        // Production: use same origin (backend and frontend on same domain)
        if (import.meta.env.PROD || window.location.hostname === 'testnotifier.co.uk') {
          return window.location.origin;
        }
        
        // Development: check if VITE_API_BASE_URL is set
      ...
      const oauthUrl = `${backendUrl}/api/auth/google?state=${encodeURIComponent(finalRedirect)}`;

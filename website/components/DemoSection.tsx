...
import { getVideoById } from "../config/videoContent";

export function DemoSection() {
  const [isVideoModalOpen, setIsVideoModalOpen] = useState(false);
  const mainDemoVideo = getVideoById('main-demo');
  ...
  return (
    <section id="demo-section" className="demo-section relative py-20 px-6 bg-gradient-to-b from-gray-50 to-white overflow-hidden">
      {/* Background Pattern */}
        ...
        <div className="max-w-5xl mx-auto mb-16">
          <div className="relative group cursor-pointer" onClick={handleVideoClick}>
            ...
            <div className="relative aspect-video bg-gradient-to-br from-gray-900 to-gray-800 rounded-3xl overflow-hidden border-4 border-white shadow-2xl">
              {/* Thumbnail */}
              {mainDemoVideo?.thumbnail && (
                <img 
                  src={mainDemoVideo.thumbnail} 
                  alt={mainDemoVideo.title}
              ...
              {mainDemoVideo?.duration && (
                <div className="absolute top-8 left-8 bg-black/50 backdrop-blur-xl border border-white/20 rounded-full px-6 py-3 text-white">
                  ...
                  <div className="text-2xl">{mainDemoVideo.duration}</div>
                </div>
              )}
      ...
      <VideoModal
        ...
        youtubeId={mainDemoVideo?.youtubeId}
        vimeoId={mainDemoVideo?.vimeoId}
        src={mainDemoVideo?.src}
        title={mainDemoVideo?.title}
        description={mainDemoVideo?.description}
      />
    </section>
  );
}

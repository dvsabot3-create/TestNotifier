...
class SecureConfig {
  ...
  validateEnvironment() {
    ...
    // Only validate optional vars in production
    if (process.env.NODE_ENV === 'production') {
      const missingOptional = optionalVars.filter(varName => !process.env[varName]);
      if (missingOptional.length > 0) {
        console.warn(`Warning: Missing optional environment variables: ${missingOptional.join(', ')} for production`);
      }
    } else {
      // Warn about missing optional vars in development
      const missingOptional = optionalVars.filter(varName => !process.env[varName]);
      if (missingOptional.length > 0) {
        console.log(`ℹ️  Development mode: Some optional features disabled: ${missingOptional.join(', ')}`);
      }
    }
